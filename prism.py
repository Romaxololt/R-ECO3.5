...
def R_ECO3inf():
    return {
        ...
        "manual": (
            ...
            "ROOTS\n"
            "  sqrt <x> [n=2]              n-th root of x (default: square root)\n"
            "  cbrt <x>                    cube root (handles negatives)\n\n"
            "POWERS & LOGARITHMS\n"
            "  pow  <x> <n>                x to the power n\n"
            "  log  <x> [base=e]           logarithm (default: natural log)\n"
            "  log2 <x>                    base-2 logarithm\n"
            "  log10 <x>                   base-10 logarithm\n"
            "  exp  <x>                    e^x\n\n"
            "TRIGONOMETRY  (add 'deg' as last arg for degrees input/output)\n"
            "  sin / cos / tan <x> [deg]\n"
            "  asin / acos / atan <x> [deg]\n"
            "  atan2 <y> <x> [deg]         4-quadrant arc tangent\n\n"
            "HYPERBOLIC\n"
            "  sinh / cosh / tanh <x>\n"
            "  asinh / acosh / atanh <x>\n\n"
            "ARITHMETIC\n"
            "  abs <x>                     absolute value\n"
            "  ceil / floor <x>            ceiling / floor\n"
            ...
            "  beta <a> <b>                B(a,b) — beta function\n\n"
            "STATISTICS\n"
            "  sum / avg / median / min / max  <x1> <x2> ...\n"
            "  variance <x1> <x2> ...      population variance\n"
            "  stddev <x1> <x2> ...        population standard deviation\n"
            "  range <x1> <x2> ...         max - min\n\n"
            "CALCULUS  (expressions use x as variable)\n"
            "  deriv <expr> <x> [order=1]  numerical derivative at x\n"
            "  integral <expr> <a> <b> [n] definite integral from a to b\n"
            "  limit <expr> <x>            numerical limit at x\n"
            "  root <expr> <x0>            root of f(x)=0 via Newton-Raphson\n"
            "  taylor <expr> <x0> <n> [x]  Taylor expansion of order n at x0\n\n"
            "CONSTANTS\n"
            "  pi   tau   e   phi\n\n"
            "CONVERSIONS\n"
            "  deg2rad / rad2deg <x>\n"
            "  hex2dec / dec2hex / dec2bin / bin2dec / dec2oct / oct2dec\n\n"
            "EXPRESSION EVALUATOR\n"
            "  eval <expr> [x=val]         evaluate any expression\n"
            "  Supported: + - * / ^ %  sin cos tan sqrt log exp abs ... pi e tau phi\n\n"
            "EXAMPLES\n"
            "  prism sqrt 9                → 3\n"
            "  prism sqrt 5 3              → cube root of 5\n"
            "  prism deriv x^2 3           → 6\n"
            "  prism deriv sin(x) 0        → 1\n"
            "  prism deriv x^3 2 2         → second derivative → 12\n"
            "  prism integral x^2 0 3      → 9\n"
            "  prism integral sin(x) 0 pi  → 2\n"
            "  prism limit sin(x)/x 0      → 1\n"
            "  prism root x^2-4 1          → 2\n"
            "  prism taylor sin(x) 0 5     → Taylor coefficients\n"
            "  prism taylor sin(x) 0 5 1   → approx sin(1)\n"
            "  prism comb 10 3             → 120\n"
            "  prism fib 10                → 55\n"
            "  prism stddev 2 4 4 4 5 5 7 9 → 2\n"
            "  prism eval sin(pi/6)        → 0.5\n"
            "  prism eval x^2+1 x=5        → 26\n"
            "  prism gamma 5               → 24  (= 4!)\n"
        )
    }