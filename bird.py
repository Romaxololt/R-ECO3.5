...
def R_ECO3dep():
    """Retourne les dépendances minimales requises pour l'initialisation du module."""
    return {
        "python": ">=3.5",
        "packages": ["colorama"],
    }
...
def R_ECO3inf():
    ...
    return {
        ...
        "L2Module": False,
        "manual": (
            "bird — fonction d'affichage avec markup ANSI\n\n"
            "bird() est un remplacement de print() qui interprète un markup de balises entre crochets.\n\n"
            "── Balises de base ──────────────────────────────────────────\n"
            "  [bold]texte[/]                  → gras\n"
            "  [red]texte[/]                   → rouge\n"
            "  [bold red bg_blue]texte[/]       → combinaison\n\n"
            "── Couleurs 256 et RGB ──────────────────────────────────────\n"
            "  [color=196]texte[/]             → couleur 256 (texte)\n"
            "  [bg_color=22]texte[/]           → couleur 256 (fond)\n"
            "  [rgb=255,100,0]texte[/]         → couleur RGB (texte)\n"
            "  [bg_rgb=0,50,100]texte[/]       → couleur RGB (fond)\n\n"
            "── Balises disponibles ──────────────────────────────────────\n"
            "  Poids     : bold, dim, italic, underline, blink, reverse, strike\n"
            "  Couleurs  : black, red, green, yellow, blue, magenta, cyan, white\n"
            "              + variantes bright_*\n"
            "  Fond      : bg_black, bg_red, … bg_white + variantes bg_bright_*\n\n"
            "── Échappement ──────────────────────────────────────────────\n"
            "  \\[ et \\]  → crochets littéraux dans le texte\n\n"
            "── Respect de l'environnement ───────────────────────────────\n"
            "  NO_COLOR=1 ou TERM=dumb ou stdout non-tty → ANSI désactivé\n\n"
            "── Helpers ──────────────────────────────────────────────────\n"
            "  bird.rule(title='', char='─', width=None, style='dim')\n"
            "    → séparateur horizontal optionnellement titré\n\n"
            "  bird.table(rows, headers=None, header_style='bold', border_style='dim')\n"
            "    → tableau ASCII avec alignement auto, markup supporté dans les cellules\n\n"
            "── Paramètres acceptés (compatibilité rich-like) ────────────\n"
            "  style, markup, justify, overflow, no_wrap, emoji,\n"
            "  highlight, width, height, crop, soft_wrap, new_line_start\n\n"
            "── Utilisation via apix ─────────────────────────────────────\n"
            "  code, fn = apix.R_ECO3('run bird')\n"
            "  fn('[bold green]OK[/] fichier écrit')\n"
            "  fn.rule('Résultats', style='bold cyan')\n"
            "  fn.table(['Alice','Bob'], headers=['Nom'])\n\n"
            "Dépendance externe : colorama (pip install colorama)"
        ),
    }