...
def R_ECO3inf():
    return {
        ...
        "manual": (
            ...
            "    Runs the command registered under the given captor via spider -vr.\n"
            ...
            "  bee delete <captor>\n"
            "    Removes a captor's command from the database.\n\n"
            "  bee show <captor>\n"
            "    Displays the command, run count, and last execution time for a captor.\n\n"
            "  bee add <name>\n"
            "    Manually adds a named entry to the available registry.\n\n"
            "  bee available\n"
            "    Displays all available entries with bound status and run count.\n\n"
            "  bee stat\n"
            "    Full statistics panel: run count and last 5 timestamps per entry.\n\n"
            "DATABASE KEYS:\n"
            "  §sys -> bee:auto:<captor>.cmd  →  command string\n"
            "  §sys -> bee:available.all      →  <RECO_SEP:=:>-separated entry names\n"
            "  §sys -> bee:stats:<name>       →  '<count>||<ts1>,<ts2>,...'\n"
        ),
    }
